...
def initialize_channel():
    if len(sys.argv) > 2:
        ...
        print("Example: python3 damp_g1_dex3_hands.py eth0")
        sys.exit(1)
...
def main():
    print("WARNING: This publishes Dex3 hand damping commands for both hands.")
    print("It sets kp=0, nonzero kd, dq=0, tau=0 at the current hand pose.")
    print("The hands will not be position-held. Keep fingers and objects clear.")
    ...
    left_command = make_damping_command(current_q(state["left"]))
    right_command = make_damping_command(current_q(state["right"]))

    print(f"Publishing hand damping commands for {PUBLISH_DURATION:.1f} seconds.")
    ...
    print("Done. Dex3 damping command sent and script exited.")
